...
{
    public class OriginalLayerWithAnchors :Step<List<Node>,Hierarchies>
    {
        ...
        public override string ChineseDescription
        {
            get
            {
                ...
                foreach (string name in anchorNames)
                    s += name + ";";
                return $"原始算法方向：{directionDescriptions[direction]} 1层锚点：{s}";
            }
        }
        ...
        public OriginalLayerWithAnchors(List<string> anchors, int direction = 1, bool iterate = false)
        {
